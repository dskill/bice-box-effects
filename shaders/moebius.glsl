...
{
	p -= offs;
	float a = atan(p.y, p.x)/6.283 + iRMSTime/32.;
	float d = length(p);
...
vec2 ispiralZoom(vec2 p, vec2 offs, float n, float spiral, float zoom, vec2 phase)
{
    ...
    p.x = (p.x - iRMSTime/32.) * 6.283;             // ( p.x, p.y )  = ( a, log(d) )
...
{
    q = Mobius(q, Z1, Z2);
    return spiralZoom(q, vec2(-.5), 5., 3.14159*.2, .5, vec2(-1, 1)*iRMSTime*.25);
}
float DrawInvMobiusSpiral(vec2 q, vec2 uv0, float r) // total inverse transform + draw iso-X
{
    vec2 v; float s = 0.;
    q = ispiralZoom(q, vec2(-.5), 5., 3.14159*.2, .5, vec2(-1, 1)*iRMSTime*.25);
    v = iMobius(q, Z1, Z2);  

...
void main() {
    ...
    hsvColor.z *= 0.9 + .1*(u_fluidData[1] + .1);
...
const sketch = function (p) {
    ...
    p.draw = () => {
        ...
        if (p.customMessage) {
            feedback.setUniform('u_fluidData', p.customMessage.values);
        }
    ...
    p.windowResized = () => {
        ...
        pingPong[0] = p.createFramebuffer({ width: p.width, height: p.height, depth: false, antialias: false });
        pingPong[1] = p.createFramebuffer({ width: p.width, height: p.height, depth: false, antialias: false });
    };
};
